...
match(_, any).
trace_expression('Main', Main) :- Main=(star((not_abcd:eps))*app(A, [0])), A=gen(['n'], ((a_match:eps)*(star((not_abcd:eps))*(app(A, [(var('n')+1)])\/app(B, [(var('n')+1)]))))), B=gen(['n'], ((b_match:eps)*app(C, [var('n')]))), C=gen(['n'], (star((not_abcd:eps))*((c_match:eps)*app(D, [var('n')])))), D=gen(['n'], guarded((var('n')>0), (star((not_abcd:eps))*((d_match:eps)*app(D, [(var('n')-1)]))), 1)).
